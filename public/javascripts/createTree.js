...
var coordSystem = function coordSystem(vec3) {
...
var randPct = function randPct(v, p) {
  if (p == 0) {
    return v;
  }
...
var createBranch = function createBranch(branchAt, branchSys, branchLength, branchTaper, branchSlices, bowFreq, bowHeight, branchRadius, scene) {
  ...
  for (var d = 0; d < branchSlices; d++) {
    ...
    _core_point.addInPlace(branchSys.x.scale(bowHeight * Math.exp(-_d_slices_length) * Math.sin(bowFreq * _d_slices_length * Math.PI))); //set core point start at spur position.


    _core_point.addInPlace(branchAt);

    _core_path[d] = _core_point; //randomize radius by +/- 40% of radius and taper to branchTaper*100% of radius  at top

    _xsr = branchRadius * (1 + (0.4 * Math.random() - 0.2)) * (1 - (1 - branchTaper) * _d_slices_length);

    _radii.push(_xsr); //determine the paths for each vertex of the cross sectional polygons


    for (var a = 0; a < _a_sides; a++) {
      _theta = a * Math.PI / 6; //path followed by one point on cross section in branchSys.y direction

      _path = branchSys.x.scale(_xsr * Math.cos(_theta)).add(branchSys.z.scale(_xsr * Math.sin(_theta))); //align with core
  ...
  var _branch = BABYLON.MeshBuilder.CreateRibbon("branch", {
    pathArray: _cross_section_paths,
    closeArray: true
  }, scene);
...
var createTreeBase = function createTreeBase(trunkHeight, trunkTaper, trunkSlices, boughs, forks, forkAngle, forkRatio, bowFreq, bowHeight, scene) {
  ...
  var _turn, _bough_turn, _bough_top, _bough; //create new branch at top of trunk for number of forks
  ...
  for (var f = 0; f < forks; f++) {
    ...
    _bough_top = _bough_core_path[_bough_core_path.length - 1]; //store branch details
    ...
    if (boughs > 1) {
      ...
      for (var k = 0; k < forks; k++) {
        ...
        _bough = createBranch(_bough_top, _bough_sys, trunkHeight * forkRatio * forkRatio, trunkTaper, trunkSlices, bowFreq, bowHeight * _PHI * _PHI, trunkTaper * trunkTaper, scene); //store branch details
...
var createTree = function createTree(trunkHeight, trunkTaper, trunkSlices, trunkMaterial, boughs, forks, forkAngle, forkRatio, branches, branchAngle, bowFreq, bowHeight, leavesOnBranch, leafWHRatio, leafMaterial, scene) {
  ...
  _base.tree.material = trunkMaterial; //create one leaf
  ...
  var _leaf = BABYLON.MeshBuilder.CreateDisc("leaf", {
    radius: _leaf_width / 2,
    tessellation: 12,
    sideOrientation: BABYLON.Mesh.DOUBLESIDE
  }, scene); //create solid particle system for leaves


  var _leaves_SPS = new BABYLON.SolidParticleSystem("leaveSPS", scene, {
    updatable: false
  }); //function to position leaves on base tree


  var _set_leaves = function _set_leaves(particle, i, s) {
  ...
  _leaves_SPS.addShape(_leaf, 2 * leavesOnBranch * Math.pow(forks, boughs), {
  ...
  _leaf.dispose(); //create SPS to use with base tree mesh base.tree


  var _mini_trees_SPS = new BABYLON.SolidParticleSystem("miniSPS", scene, {
    updatable: false
  }); //create SPS to use with leaves mesh


  var _mini_leaves_SPS = new BABYLON.SolidParticleSystem("minileavesSPS", scene, {
    updatable: false
  ...
  var _set_mini_trees = function _set_mini_trees(particle, i, s) {
  ...
  var _set_branches = function _set_branches(particle, i, s) {
    var _a = _places[s][0];
    var _b = _places[s][1];
    var _mini_sys = _base.directions[_a];
    ...
    var treeOffsetX = "150px";
    var treeOffsetY = "-100px"; // {{!-- createTooltip(particle, scene, treeOffsetX, treeOffsetY, testText); --}}
  }; //add base tree mesh enough for all the final forked branches


  _mini_trees_SPS.addShape(_base.tree, Math.pow(forks, boughs + 1), {
    positionFunction: _set_mini_trees
  }); //add base tree mesh given the number of branches in that parameter.


  _mini_trees_SPS.addShape(_base.tree, branches, {
    positionFunction: _set_branches
  });

  var _tree_crown = _mini_trees_SPS.buildMesh(); // form mesh with all mini trees


  _tree_crown.material = trunkMaterial; //add leaves mesh enough for all the final forked branches

  _mini_leaves_SPS.addShape(_leaves, Math.pow(forks, boughs + 1), {
    positionFunction: _set_mini_trees
  }); //add leaves mesh given the number of branches in that parameter.


  _mini_leaves_SPS.addShape(_leaves, branches, {
    positionFunction: _set_branches
  });
  ...
  _leaves_crown.material = leafMaterial; //Give the three mesh elements of full tree the same parent.
